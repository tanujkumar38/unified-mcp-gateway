 ...
 */
export function buildFinancialModel(intake) {
    ...
    const createScenario = (name, utilPct, custPerDay, aov) => {
    ...
    const calculateSensitivity = (label, variationStr, simulatedProfit, rank) => {
    ...
    const sensitivityAnalysis = rawSensitivity.map((item, idx) => calculateSensitivity(item.label, item.variation, item.profit, idx + 1));
    return {
        currency,
        capexItems,
 ...
 */
export function buildFeasibilityScorecard(intake, financials) {
    const dimensions = [
        {
 ...
 */
export function buildExecutionRoadmap() {
    return {
        days1To30: [
            {
