...
export function generateRatingVsReviewsSvg(competitors: CompetitorProfile[]): string {
  const width = 640;
  const height = 210;
  const padLeft = 55;
  const padRight = 30;
  ...
  const getX = (r: number) => padLeft + ((r - minRating) / (maxRating - minRating)) * chartW;
  const getY = (c: number) => padTop + chartH - (c / maxReviews) * chartH;
...
export function generateCustomerComplaintSvg(voc: VectorDResult): string {
  const width = 640;
  const height = 240;
  const padLeft = 210;
  const padRight = 60;
...
export function generateRevenueScenarioSvg(financials: FinancialModelResult): string {
  const width = 640;
  const height = 260;
  const padLeft = 80;
  const padRight = 40;
  ...
  const getH = (val: number) => Math.max(0, (val / maxVal) * chartH);
...
export function generateSensitivityAnalysisSvg(financials: FinancialModelResult): string {
  const width = 640;
  const height = 230;
  const padTop = 38;
  const barHeight = 16;
...
export function generateRiskHeatmapSvg(risks: RiskMatrixItem[]): string {
  const width = 640;
  const height = 240;
  const padLeft = 90;
  const padBottom = 40;
...
export function generateTreeOfThoughtsSvg(tot: TreeOfThoughtsResult): string {
  const width = 640;
  const height = 280;
