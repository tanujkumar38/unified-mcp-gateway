...
import { z } from "zod";
import {
  IntakeRequirementsSchema,
  IntakeRequirements,
  MasterMarketAuditResult,
  SourceRegisterItem,
  AssumptionRegisterItem,
  DataQualityReport
} from "./types.js";
import {
  validateAndEnforceIntake,
  auditVectorAGeospatial,
  auditVectorBMenuPricing,
  auditVectorCMultimodal,
  auditVectorDVoiceOfCustomer,
  auditVectorEMarketDemand,
  auditVectorFDigitalDemand,
  analyzeMarketGaps
} from "./engine/research.js";
import {
  executeTreeOfThoughts,
  conductDevilsAdvocateStressTest,
  simulateSecondOrderDynamics,
  calculateBayesianPosterior,
  conductCriticalThinkingAudit,
  deconstructFirstPrinciples,
  conductInversionPreMortem,
  exploreLateralThinking,
  simulateGameTheoryWarRoom,
  synthesizeDialectics,
  simulateCounterfactuals,
  executeOmniCognitiveAudit
} from "./engine/reasoning.js";
import {
  buildFinancialModel,
  buildFeasibilityScorecard,
  buildRiskMatrix,
...
export function registerApexTools(server: McpServer) {
  ...
  server.tool(
    "intake_requirements",
    "Step 0: Validates and enforces mandatory and optional market research parameters, checks catchment radius, identifies data gaps, and establishes research constraints.",
    IntakeRequirementsSchema.shape,
    async (params) => {
  ...
  server.tool(
    "audit_geospatial_competitors",
    "Vector A: Conducts hyper-local competitor discovery across 3-5km catchment, categorizes direct/indirect/benchmark competitors, calculates ratings, review counts, distances, operating hours, and provides Online-Only Confidence Adjustment matrix.",
    IntakeRequirementsSchema.shape,
    async (params) => {
  ...
  server.tool(
    "audit_menu_pricing",
    "Vector B: Analyzes competitor product menus, calculates price distribution (entry, core, premium, median), maps price bands, identifies gaps, and calculates Estimated Gross Margin Proxy.",
    IntakeRequirementsSchema.shape,
    async (params) => {
      const { intake } = validateAndEnforceIntake(params);
  ...
  server.tool(
    "audit_multimodal_infrastructure",
    "Vector C: Analyzes physical environment, seating capacity, acoustics, power outlets, Wi-Fi, and parking, classified into [Observed], [Inferred], or [Unknown].",
    IntakeRequirementsSchema.shape,
    async (params) => {
      const { intake } = validateAndEnforceIntake(params);
  ...
  server.tool(
    "audit_voice_of_customer",
    "Vector D: Samples 100+ customer reviews across 6 dimensions (Product, Service, Pricing, Infrastructure, Experience, Positive Drivers), computing complaint frequency and market pain points.",
    IntakeRequirementsSchema.shape,
    async (params) => {
      const { intake } = validateAndEnforceIntake(params);
  ...
  server.tool(
    "audit_market_demand",
    "Vector E: Analyzes catchment demographics, daytime working population, student density, residential vs commercial split, demand generators, and seasonal fluctuations.",
    IntakeRequirementsSchema.shape,
    async (params) => {
      const { intake } = validateAndEnforceIntake(params);
  ...
  server.tool(
    "audit_digital_demand",
    "Vector F: Analyzes search volume intent, Google Maps local ranking difficulty, competitor digital maturity, and customer acquisition gaps.",
    IntakeRequirementsSchema.shape,
    async (params) => {
      const { intake } = validateAndEnforceIntake(params);
      const geo = auditVectorAGeospatial(intake);
  ...
  server.tool(
    "analyze_market_gaps",
    "Synthesizes and triangulates findings into at least 3 evidence-backed white-space market opportunities with competitive saturation, monetization, difficulty, and confidence.",
    IntakeRequirementsSchema.shape,
    async (params) => {
      const { intake } = validateAndEnforceIntake(params);
      const geo = auditVectorAGeospatial(intake);
  ...
  server.tool(
    "deliberate_tree_of_thoughts",
    "Cognitive Layer: Explores and deliberates across 4 competing strategic business archetypes, scores each across 5 quantitative criteria, prunes sub-optimal paths, and synthesizes the Pareto-optimal strategy.",
    IntakeRequirementsSchema.shape,
    async (params) => {
      const { intake } = validateAndEnforceIntake(params);
      const geo = auditVectorAGeospatial(intake);
  ...
  server.tool(
    "conduct_devils_advocate_audit",
    "Cognitive Layer: Hostile red-team evaluation stress-testing the business model against aggressive incumbent attacks, critical failure conditions, cognitive biases, and kill-switch criteria.",
    IntakeRequirementsSchema.shape,
    async (params) => {
      const { intake } = validateAndEnforceIntake(params);
      const geo = auditVectorAGeospatial(intake);
  ...
  server.tool(
    "simulate_second_order_effects",
    "Cognitive Layer: Models systemic ripple effects, feedback loop polarity (reinforcing vs balancing), and unintended consequences of key pricing, space, and operational decisions.",
    IntakeRequirementsSchema.shape,
    async (params) => {
      const { intake } = validateAndEnforceIntake(params);
      const geo = auditVectorAGeospatial(intake);
  ...
  server.tool(
    "synthesize_bayesian_feasibility",
    "Cognitive Layer: Updates prior industry baseline survival rates with empirical local evidence vectors (Vectors A-F) to calculate posterior survival probability and credible intervals.",
    IntakeRequirementsSchema.shape,
    async (params) => {
      const { intake } = validateAndEnforceIntake(params);
      const geo = auditVectorAGeospatial(intake);
  ...
  server.tool(
    "audit_critical_thinking",
    "Cognitive Layer: Conducts rigorous Socratic interrogation of core business assumptions, exposes logical fallacies, mitigates cognitive biases (optimism, survivorship, anchoring), and rates epistemic credibility.",
    IntakeRequirementsSchema.shape,
    async (params) => {
      const { intake } = validateAndEnforceIntake(params);
      const geo = auditVectorAGeospatial(intake);
  ...
  server.tool(
    "deconstruct_first_principles",
    "Cognitive Layer: Deconstructs the business down to fundamental thermodynamic, spatial, and economic truths (raw coffee extraction yield, seat-hour yield physics, zero-based cost reconstruction) rather than reasoning by analogy.",
    IntakeRequirementsSchema.shape,
    async (params) => {
      const { intake } = validateAndEnforceIntake(params);
      const geo = auditVectorAGeospatial(intake);
  ...
  server.tool(
    "conduct_inversion_pre_mortem",
    "Cognitive Layer: Charlie Munger style prospective hindsight autopsy assuming 100% capital loss at Month 24, forensic analysis of failure mechanisms, 5 Anti-Goals, and mandatory operational kill-switches.",
    IntakeRequirementsSchema.shape,
    async (params) => {
      const { intake } = validateAndEnforceIntake(params);
      const geo = auditVectorAGeospatial(intake);
  ...
  server.tool(
    "explore_lateral_thinking",
    "Cognitive Layer: Edward de Bono Provocative Operations (PO), cross-industry innovation transfers (gym subscriptions, airline yield management), and asymmetric revenue discovery.",
    IntakeRequirementsSchema.shape,
    async (params) => {
      const { intake } = validateAndEnforceIntake(params);
      const geo = auditVectorAGeospatial(intake);
  ...
  server.tool(
    "simulate_game_theory",
    "Cognitive Layer: Evaluates competitor reaction functions across specialty roasteries, mass kiosks, and local diners, solves for Nash equilibrium defensibility, and formulates minimax moats.",
    IntakeRequirementsSchema.shape,
    async (params) => {
      const { intake } = validateAndEnforceIntake(params);
      const geo = auditVectorAGeospatial(intake);
  ...
  server.tool(
    "synthesize_dialectics",
    "Cognitive Layer: Resolves core commercial polarities (Student Affordability vs Specialty Margin, Dwell Time vs Seat Turnover, Silence vs Hospitality) via Hegelian Thesis-Antithesis-Synthesis.",
    IntakeRequirementsSchema.shape,
    async (params) => {
      const { intake } = validateAndEnforceIntake(params);
      const geo = auditVectorAGeospatial(intake);
  ...
  server.tool(
    ...
    "Cognitive Layer: Simulates alternate branch realities, macro/micro shock stresses (academic disruption, commodity inflation, hostile landlord escalation), and scores systemic antifragility.",
    IntakeRequirementsSchema.shape,
    async (params) => {
      const { intake } = validateAndEnforceIntake(params);
      const financials = buildFinancialModel(intake);
  ...
  server.tool(
    "execute_omni_cognitive_audit",
    "Cognitive Layer Master: Executes all 10 thinking paradigms (Critical Thinking, First-Principles, Inversion, Lateral Thinking, Game Theory, Dialectics, Counterfactuals, System Dynamics, Tree of Thoughts, and Bayesian Updating) in a unified multi-angle cognitive audit.",
    IntakeRequirementsSchema.shape,
    async (params) => {
      const { intake } = validateAndEnforceIntake(params);
      const geo = auditVectorAGeospatial(intake);
  ...
  server.tool(
    ...
    "Builds location-specific CAPEX (Low/Base/High/Basis/Confidence), OPEX breakdown (Fixed vs Variable), Unit Economics, 3 Scenarios (Conservative, Base, Upside), and 8-factor Sensitivity Analysis.",
    IntakeRequirementsSchema.shape,
  ...
  server.tool(
    "generate_feasibility_scorecard",
    "Calculates the weighted 100-point feasibility scorecard and compiles the 12-factor Risk Matrix with probability, impact, early warning signals, and mitigation.",
    IntakeRequirementsSchema.shape,
    async (params) => {
      const { intake } = validateAndEnforceIntake(params);
      const financials = buildFinancialModel(intake);
  ...
  server.tool(
    ...
    "Generates Deliverable 1: Comprehensive 10-section Master Markdown Report (.md) with Source Register, Assumption Register, and Data Quality & Confidence Report.",
    IntakeRequirementsSchema.extend({
  ...
  server.tool(
    ...
    "Generates Deliverable 2: Publication-quality Executive PDF report with embedded SVG data charts, KPI cards, styled tables, and automated visual QA.",
    IntakeRequirementsSchema.extend({
  ...
  server.tool(
    ...
    "Autonomous Master Orchestrator: Conducts all 6 research vectors, triangulates findings, builds financial models, and exports both Deliverable 1 (Markdown) and Deliverable 2 (Executive PDF).",
    IntakeRequirementsSchema.extend({
 ...
 */
export function runCompleteAuditInternal(params: Partial<IntakeRequirements>): MasterMarketAuditResult {
  const { intake } = validateAndEnforceIntake(params);
  ...
  const omniCognitive = executeOmniCognitiveAudit(
    intake,
    geospatial,
    pricing,
    voiceOfCustomer,
  ...
  const sourceRegister: SourceRegisterItem[] = [
    {
  ...
  const assumptionRegister: AssumptionRegisterItem[] = [
    {
